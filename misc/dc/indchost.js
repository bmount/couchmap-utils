...
function inDChost() {
  var dchost = false;
  var dcid = document.location.href.match(/[a-f0-9]{34}/);
  console.log(dcid);
  return dcid[0];
}
...
function gotFirstDoc(data) {
  ...
  $.get(config.host + "/db/"+inDChost()+"/_design/polymapper/_rewrite/api/" + firstDoc.id, function(moarData){
...
function fetchFeatures(bbox, callback) {
  $.ajax({
    url: config.host + "/db/"+inDChost()+"/_design/polymapper/_rewrite/data",
    data: {
      "bbox": bbox
    },
    success: callback
...
$(function(){  
  ...
  $.get( config.host + "/db/"+inDChost()+"/_all_docs?limit=10", gotFirstDoc); 
});